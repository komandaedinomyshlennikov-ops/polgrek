...
(function () {
  ...
  const UI = isEn
    ...
    : {
        ...
        allBooks: 'Все книги →',
        doorsLabel: 'Книга под то, что мешает',
        ...
        searchEmpty: 'Ничего. Другое слово — или сбросьте фильтр.',
  ...
  function mountBookQuiz() {
    ...
    const steps = isEn
      ...
      : [
          {
            q: 'Что сейчас громче всего?',
            options: [
              { id: 'burnout', label: 'Выгорание — выходные не помогают', slug: 'reset' },
              { id: 'fog', label: 'Туман · память после 40', slug: 'mozg-na-100' },
              { id: 'stress', label: 'Стресс · тревога не выключается', slug: 'stress-i-mozg' },
              { id: 'bio', label: 'Пробовал «прокачать» мозг — и выгорел', slug: 'biohacking-mozga' },
              { id: 'money', label: 'Деньги · импульсы · ресурс на дне', slug: 'mozg-i-dengi' },
            ],
          },
          {
            q: 'С чего начнём?',
            options: [
              { id: 'excerpt', label: 'С главы — потом решу' },
              { id: 'book', label: 'Сразу вся книга на Литрес' },
          ...
          },
          {
            ...
            options: [
              ...
              { id: 'deep', label: 'Полный разбор с уровнями A–D' },
            ],
          },
        ];
  ...
  const HOME_SEGMENTS = {
    default: {
      ...
      cardTitle: 'Сначала текст. Потом — если зайдёт',
      cardBody: 'Глава здесь. Вся книга — на Литрес. Платить заранее не надо.',
      book: 'mozg-na-100',
      why: null,
      checks: [],
    },
    burnout: {
      ...
      leadHtml:
        'Сначала <strong>проверить себя</strong> и главу. RESET — если нужен полный разбор после истощения.',
      cardTitle: 'Вход: восстановление',
      cardBody: 'Глава RESET здесь. Вся книга — на Литрес, если зайдёт.',
      book: 'reset',
      why: 'burnout',
    ...
    fog40: {
      ...
      leadHtml:
        'Сначала <strong>глава</strong>. «Мозг на 100+» — если нужен спокойный план, а не хайп.',
      cardTitle: 'Вход: ясность',
    ...
    stress: {
      ...
      leadHtml:
        'Проверьте себя → разбор «почему тревога кормит себя» → <strong>глава</strong>.',
      cardTitle: 'Вход: стресс',
      cardBody: 'Отрывок «Стресс и мозг». Без «просто выдохни».',
      book: 'stress-i-mozg',
      why: 'stress',
    ...
    energy: {
      ...
      leadHtml:
        'Сначала <strong>разбор на странице</strong>, потом глава. Книга — если нужен полный текст.',
      cardTitle: 'Вход: энергия',
      cardBody: 'Отрывок «Анатомия энергии» / RESET — здесь.',
    ...
    money: {
      ...
      leadHtml:
        'Не про «характер». Про <strong>состояние</strong>. Глава — потом сами решите, нужна ли вся книга.',
      cardTitle: 'Вход: решения и импульсы',
      cardBody: 'Отрывок «Мозг и деньги» здесь.',
      book: 'mozg-i-dengi',
      why: 'money',
    ...
    bio: {
      ...
      leadHtml:
        'Сначала <strong>глава</strong>. «Биохакинг мозга» — если нужен честный фильтр мифов.',
      cardTitle: 'Вход: фокус и протоколы',
      cardBody: 'Отрывок здесь. У советов — уровни A–D.',
        ...
        if (bridgeText) {
          if (active) {
            bridgeText.innerHTML = isEn
              ...
              : `Отметили <strong>${n}</strong> — вы не «сломаны» и не одни. Это нормально.`;
          } else {
            bridgeText.innerHTML = isEn
              ? `Mark the items above — if at least <strong>three</strong> feel familiar, you are not alone.`
              : `Отметьте сверху — если знакомо <strong>хотя бы три</strong>, вы не «сломаны» и не одни.`;
          }
        }
      }
