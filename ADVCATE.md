...
## Post-deploy smoke (buy path)

После каждого деплоя (5–10 мин):

1. **Каталог** `/books/index.html` — у всех 13 карточек видна кнопка «Купить на Литрес» (не «пустые» карточки).
2. **Home flagships** — 3 кнопки покупки на флагманах.
3. **Product** `/books/mozg-na-100.html` — primary CTA «Купить на Литрес».
4. Клик (или copy URL): в ссылке есть `utm_source=advcake`, `sub1=` = slug книги (не `stres&mozg=`).
5. `rel` на партнёрских ссылках включает `sponsored` (если смотрите исходник/инспектор).
6. iPhone Safari + Android Chrome: кнопки **видны** без необходимости «перезагрузить дважды».
7. NSF-фильтр на каталоге: переключение «Все / [A]…» не прячет кнопки навсегда.
8. Метрика (если настроена): goals `litres` / `excerpt_download` при клике.

Ожидаемый шаблон URL:
```
https://www.litres.ru/book/...?utm_source=advcake&...&sub1=SLUG&erid=...
```

Конфиг: `js/data.js` → `affiliate` (+ `bySlug` при исключениях).
